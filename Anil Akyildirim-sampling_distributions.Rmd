...
Based on the summary and histogram of the data, it is normal distribution very sligtly skewed to the right. 

```{r}
library('DATA606')
sd <- sd(area)
normalPlot(mean = 1500, sd=sd)
```
...
The mean value is slightly higher than the population mean. 

```{r}
sd_2 <- sd(samp1)
normalPlot(mean = 1564, sd = sd_2)
```
...
The mean of samp2 is slighly lower the the population mean and slightly lower than samp1. The higher the sample size the more accurate estimate of the mean we would get. It will be closer to the population mean. 
...
m_50 <- mean(sample_means50)
sd_50 <- sd(sample_means50)
normalPlot(mean = m_50, sd=sd_50)
qqnorm(sample_means50)
qqline(sample_means50)

```

It is nearly normal distribution, it is not skewed to the right or left and unimodal. 

Yes. If we collected more samples (50,000) the distribution would even move more towards the normal distribution.
...
There are 100 elements and each element is the sample mean.
...
Center gets narrower along with the spread. 
...
summary(samp_price)
sd_price <- sd(samp_price)
sd_price


```

The best point estimate of population is 175024. 
...
3 has smaller spread( the sample size 150). We want higher sample sizes and smaller spread.
